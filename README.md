...
```shell
vagrant ssh -c 'kubectl -n kubernetes-dashboard get secret/admin-user -o go-template="{{.data.token | base64decode}}"' controlplane >> .kube/controlplane-dashboard-token.txt
```

Make the dashboard accessible:
```shell
vagrant ssh -c 'kubectl proxy &' controlplane
```
